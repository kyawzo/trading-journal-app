...
| Strategy Performance (CSP, CC, spreads, etc.) | Shows which strategies are actually working over time | `Done` |
| Realized P/L & Open Exposure by account and currency | Separates closed results from current open inventory/cost exposure without assuming live market pricing | `Done` |
| Monthly Performance Trend (Options) | Helps users see consistency, drawdowns, and recovery patterns in options positions | `Done` |
| Monthly Performance Trend (Holdings) | Helps users review realized stock exits month by month using sale and called-away events | `Done` |
| Holding Performance Snapshot | Shows open stock holdings, realized from sold lots, and cost basis quality | `Done` |
...
| Import Quality Report (imported rows, failed rows, rollback events) | Builds trust in data quality and audit trail | `Done` |
| Cash Flow Report (deposits, withdrawals, premiums, fees, dividends) | Gives clear money-in/money-out visibility for each broker account | `Done` |
...
| Phase | Scope | Status |
|---|---|---|
| Phase 1 - Foundation | Trading Summary (Options), Monthly Trend (Options + Holdings), Cash Flow Report | `Done` |
| Phase 2 - Strategy & Risk | Strategy Performance, Exposure View, Realized P/L & Open Exposure baseline | `In Progress` |
